...
if(isset($_GET['start_date']) && isset($_GET['end_date'])) {
	...
	$sql = "SELECT name_company
		FROM schedule s
		INNER JOIN flight_control f ON f.id_schedule = s.id
		INNER JOIN airlines a ON a.id = s.airline
		WHERE delta_t > 0 AND 
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<p>Авиакомпания, задерживающая рейсы чаще других за выбранный период: $name_company</p>";
    } else {
